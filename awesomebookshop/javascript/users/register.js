$(document).ready(function() {
	...
	$("input[name='nif']").on('change input', function(){
    if(validaContribuinte(this.value) == 1){ // check if the NIF is valid
	   $(this).nextAll().remove();
       $(this).closest('div .form-group').addClass('has-error');
       $(this).next().remove();
       $(this).after('<span id="helpBlock" class="help-block">Deve introduzir um NIF válido</span>');
   }
   else{
   	$(this).closest('div .form-group').removeClass('has-error');
   	$(this).nextAll().remove();
